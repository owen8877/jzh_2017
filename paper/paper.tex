...
经过比较不同算法的优劣，我们最终采用了SSA(奇异谱分析)算法作为我们的预测算法。该方法是一种对一维时间序列的主成分分析方法，能够在带噪声的时间序列中有效提取信号信息。SSA的主要目标是将原来的时间序列分解为若干个趋势、震荡或噪声时间序列的和。它将已知的时间序列变形为轨迹矩阵，对其协方差矩阵运用Karhunene-Loeve分解，从而将含有延迟相空间的信息拆解开来，从而提取出趋势等规律。
\\
